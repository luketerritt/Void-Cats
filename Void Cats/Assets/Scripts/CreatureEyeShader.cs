...
public class CreatureEyeShader : MonoBehaviour
{
    ...
    void Update()
    {
        if(thisMaterial.HasProperty("_Jnt_Eye_xyz")) //"_Jnt_EyeShape_L_rx"
        {
            //Debug.Log("The code found the eyeShape on the material");
            
            //Declan updated the shader :)
            thisMaterial.SetVector("_Jnt_Eye_xyz", new Vector4(eyeShapeInput.transform.localPosition.x, eyeShapeInput.transform.localPosition.y, eyeShapeInput.transform.localPosition.z, 0));
        }
        else
        {
